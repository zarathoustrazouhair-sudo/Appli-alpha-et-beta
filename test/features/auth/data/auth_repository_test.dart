  ...
  group('AuthRepository Security Tests', () {
    ...
    test('signIn throws exception for invalid email format', () async {
      expect(
        ...
        throwsA(predicate((e) =>
            e is Exception && e.toString().contains("Format d'email invalide"))),
      );
    });
    ...
    test('signIn passes validation and attempts to use client with valid inputs', () async {
      ...
      expect(
        ...
        throwsA(predicate((e) =>
            e is Exception &&
            e.toString().contains("Une erreur est survenue lors de la connexion"))),
      );
    });
    ...
    test('signIn throws exception for email with XSS attempt', () async {
      expect(
        () => authRepository.signIn('user@domain.com<script>alert(1)</script>', 'password123'),
        throwsA(predicate((e) =>
            e is Exception && e.toString().contains("Format d'email invalide"))),
      );
    });
