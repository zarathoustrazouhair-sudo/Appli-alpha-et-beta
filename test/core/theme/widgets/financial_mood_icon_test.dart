...
void main() {
  group('FinancialMoodIcon Accessibility Tests', () {
    testWidgets(
      'Should display correct semantic label for Green status (> 3 months)',
      (tester) async {
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(body: FinancialMoodIcon(monthsOfSurvival: 4.5)),
          ),
        );
        ...
        // Semantics check
        expect(
          find.bySemanticsLabel('Santé financière excellente'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'Should display correct semantic label for Orange status (> 0 months)',
      (tester) async {
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(body: FinancialMoodIcon(monthsOfSurvival: 1.5)),
          ),
        );

        expect(find.byType(FinancialMoodIcon), findsOneWidget);
        expect(
          find.bySemanticsLabel('Santé financière stable'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'Should display correct semantic label for Red status (< 0 months)',
      (tester) async {
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(body: FinancialMoodIcon(monthsOfSurvival: -1.0)),
          ),
        );

        expect(find.byType(FinancialMoodIcon), findsOneWidget);
        expect(
          find.bySemanticsLabel('Santé financière critique'),
          findsOneWidget,
        );
